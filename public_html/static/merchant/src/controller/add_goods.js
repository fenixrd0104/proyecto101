...
var $ = layui.$
    ...
    layedit.set({
        uploadImage: {
            url: layui.setter.baseUrl+'merchant/upload/upload' //接口url
            ,type: 'post' //默认post
        }
    });
    ...
    admin.req({
        url: layui.setter.baseUrl+'merchant/goods/addGoods' //实际使用请改成服务端真实接口
        ,data:{}
        ,type:'get'
    ...
    admin.req({
        url: layui.setter.baseUrl+'merchant/goods/ajaxGetCategory' //实际使用请改成服务端真实接口
        ,data: {pid:type_id},
        type:'get',
        success: function(res){
            if(res.code==1){
    ...
    form.on('select(choose_caidan_id)',function(e){
        ...
        // 类型
        admin.req({
            url: layui.setter.baseUrl+'merchant/goods/getTypeList' //实际使用请改成服务端真实接口
            ,data: {id:id}
            ,type:'get'
            ,success: function(res){
                if(res.code==1){
    ...
    function get_data_cate(type_id){
        admin.req({
            url: layui.setter.baseUrl+'merchant/goods/ajaxGetAttr' //实际使用请改成服务端真实接口
    ...
    window.choosed = function (that) {
        ...
        var params=list;
        admin.req({
            url: layui.setter.baseUrl+'merchant/goods/ajaxGetSpecInput'
            ,data: params,
            type:'get',
            success: function(res){
                if(res.code==1){
