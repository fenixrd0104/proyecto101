...
// template engine instance
    ...
    protected $config = [
        // Default template rendering rules 1 Parse to lowercase + underscore 2 Convert all to lowercase 3 Keep the operation method
        'auto_rule' => 1,
        // view directory name
        'view_dir_name' => 'view',
        ...
        // Whether to open the template compilation cache, if set to false, it will be recompiled every time
        'tpl_cache' => true,
    ];
    ...
    public function __construct(App $app, array $config = [])
    {
        ...
        $this->template->extend('$Request', function (array $vars) {
            // Get Request request object parameters
     ...
     * @access public
     * @param string $template template file or template rule
     * @return bool
     */
     ...
     * @param array $data template variable
     * @return void
     */
    public function fetch(string $template, array $data = []): void
    {
        ...
        // Template does not exist throw exception
        if (!is_file($template)) {
    ...
    {
        // Analysis Template File Rules
        $request = $this->app['request'];

        // Get the view root directory
        if (strpos($template, '@')) {
            // call across modules
        ...
        if (0 !== strpos($template, '/')) {
            ...
            if ($controller) {
                if ('' == $template) {
                    // If the template file name is empty, locate according to the default template rendering rules
