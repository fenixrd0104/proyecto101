...
class Ad extends Base
{
    ...
    public function index(){
        ...
        $count = Db::name('ad')->alias('a')->where($map)->count();//Calculate total pages
//        $allpage = intval(ceil($count / $limits));
        $lists = Db::name('ad')
             ->alias('a')
             ->join('ad_position p','a.ad_position_id = p.id','LEFT')
             ->where($map)
             ->field('a.*,p.name,p.height,p.width')
             ->field('p.name')
             ->page($Nowpage,$limits)
             ->order('a.orderby desc')
             ->select();

        return json(['code'=>1,'data'=>['count'=>$count,'allpage'=>$Nowpage,'key'=>$key,'lists'=>$lists]]);
    ...
    public function delAd()
    {
        ...
        if($flag){
            return json(['code' => 1, 'data' => '', 'msg' => 'Ad deleted successfully']);
        }else{
            return json(['code' => 0, 'data' => '', 'msg' => 'Failed to delete ad']);
        }
    ...
    public function adState()
    {
        $id=input('param.id');
//        $id=24;
        $status = Db::name('ad')->where(array('id'=>$id))->value('status');//Judging the current state
        if($status==1)
        {
        ...
        else
        {
            ...
            return json(['code' => 1, 'data' => $flag['data'], 'msg' => 'Turned on']);
        }  
    } 
...
//        $id=input('param.id');
        $where['id']=input('param.id');
        $status = Db::name('ad_position')->where($where)->value('status');//Judging the current state
