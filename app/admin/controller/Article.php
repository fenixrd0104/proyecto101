...
use app\admin\model\PopularizeModel;
use think\facade\Db;
...
class Article extends Base
{
    ...
    public function index(){
        ...
        $count = Db::name('article')->where($map)->count();//Calculate total pages
        $lists = Db::name('article')
             ->alias('a')
             ->join('article_cate ac ','a.cate_id = ac.id')
             ->where($map)
             ->field('a.title,a.orderby,a.photo,a.id,a.views,a.status,a.create_time,a.update_time,a.is_tui,ac.name')
             ->page($Nowpage, $limits)
             ->select();
    ...
    {
        $id=input('param.id');
        $status = Db::name('article')->where(array('id'=>$id))->value('status');//Judging the current state
        if($status==1)
        {
    ...
    {
        $id=input('param.id');
        $status = Db::name('article')->where(array('id'=>$id))->value('is_tui');//Judging the current state
        if($status==1)
        {
 ...
 */
    ...
    {
        $id=input('param.id');
        $status = Db::name('article_cate')->where(array('id'=>$id))->value('status');//Judging the current state
        if($status==1)
        {
    ...
    public function index_tui(){
        ...
        $Nowpage = input('get.page') ? input('get.page'):1;
        $limits = input('get.limit',15);
        $count = Db::name('popularize')->where($map)->count();//Calculate total pages
        $lists = Db::name('popularize')
            ->where($map)
            ->page($Nowpage, $limits)
            ->select();
    ...
    public function addPostTui(){
        if(request()->isPost()){
            ...
            if(!$param['content']){
                return json(['code' => 0, 'msg' => 'Please enter the content']);
            }
            if(!$param['extend_url']){
                return json(['code' => 0, 'msg' => 'Please upload the promotion QR code']);
            }

            $article = new PopularizeModel();
            $flag = $article->insertArticle($param);
            return json(['code' => $flag['code'], 'data' => $flag['data'], 'msg' => $flag['msg']]);
        }
    ...
    public function editPostTui(){
        ...
        if(request()->isPost()){
            ...
            return json(['code'=>0,'data'=>[],'msg'=>'wrong operation']);
        }

    }

 // delete promotion
    public function deltui(){
        $id = input('param.id');
        $flag = Db::name('popularize')
            ->where('id', $id)
            ->delete();
        if($flag){
            return json(['code' => 1, 'data' => '', 'msg' => 'Promotion and deletion succeeded']);
        }else{
            return json(['code' => 0, 'data' => '', 'msg' => 'Failed to promote deletion']);
        }
    }
