...
use app\admin\model\PurchaseReceiptLogModel;
...
use app\admin\model\StockDeliveryListsModel;
use app\admin\model\StockDeliveryLogModel;
use app\admin\model\StockDeliveryModel;
...
class StockDelivery extends Base
{
    ...
    public function crateDeliveryByOrder($order_id){
       ...
       if($data){
        ...
        //add log
        StockDeliveryLogModel::operate($this->userId,$r_id,$this->userName." Create a shipping order:".$r_id);
        return json(['code'=>1,'data'=>['info'=>$info,'lists'=>$lists],'msg'=>'']);

    }

    //Create an order from a store
    public function crateReceiptByNew($to_shop){
        ...
        if(!in_array($to_shop,$shop_id_arr['data'])){
            return json(['code'=>0,'data'=>[],'msg'=>'Cant apply for this store to transfer goods']);
        }
        //Create Order
        $StockDeliveryModel= new StockDeliveryModel();
        $r_id = $StockDeliveryModel->createOrder(0,0,$to_shop,$shop_id,'');
        //add log
        PurchaseReceiptLogModel::operate($this->userId,$r_id,$this->userName."Create a shipping order:".$r_id);
