...
class GoodsBrand extends Model
{
    ...
    public static function onBeforeInsert($goodsBrand)
    {
        $GoodsCategory = new GoodsCategory();
        if ($goodsBrand->parent_cat_id) {

            $goodsBrand->cat_name = $GoodsCategory->where('id', $goodsBrand->parent_cat_id)->value('name');
        } elseif ($goodsBrand->cat_id) {
            $goodsBrand->cat_name = $GoodsCategory->where('id', $goodsBrand->cat_id)->value('name');
        }

    }

    public static function onBeforeUpdate($goodsBrand)
    {
        $GoodsCategory = new GoodsCategory();
        if ($goodsBrand->parent_cat_id) {
            $goodsBrand->cat_name =$GoodsCategory->where('id', $goodsBrand->parent_cat_id)->value('name');
