     	...
     	try{
     		if($param['pid']){
     			if($this->where(array('pid'=>$param['pid']))->count()>=5){
     				return array('code'=>0,'msg'=>'二级菜单不能超出5个');
     			}
     		}else{
     			if($this->where(array('pid'=>0))->count()>=3){
     				return array('code'=>0,'msg'=>'一级菜单不能超出3个');
     			}
     		}
     ...
     * @param 
     ...
     public function menugenerate(){
     	...
     	if ($WechatMenu->createMenu($menu_list)) {
     		return array('code'=>1,'msg'=>'创建成功，等待生效','data' => '');
     	}else{
     		return array('code'=>0,'msg'=>'创建失败!错误代码'.$WechatMenu->errCode.'错误提示'.$WechatMenu->errMsg,'data' => '');
     	}
     }
     /**
