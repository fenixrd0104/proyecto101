...
use app\merchant\model\GoodsPriceLog;
use app\merchant\model\GoodsStock;
use app\merchant\model\GoodsStockLog;
use app\merchant\model\ShopListsModel;
use app\merchant\model\SpecGoodsModel;
use app\merchant\model\StockInventoryListsModel;
use app\merchant\model\StockInventoryModel;
...
class StockInventory extends Base
{
    ...
    public function crate($type=1,$remarks=''){
       ...
       $id = $StockInventoryModel->createInventory($shop_id,$type,$this->nickname,$this->mid,$remarks);
       if(!$id){
   ...
   //Cancel
    ...
    public function yujing(){
       $res = Db::table('(SELECT goods_id,shop_id,spec_id,sum(stock) as stock FROM `think_goods_stock` group by spec_id) as goods_stock')->field('goods_stock.goods_id,goods_stock.shop_id,goods_stock.spec_id,goods_stock.stock,think_goods.goods_name,think_goods.goods_sn,think_spec_goods.spec_name,think_goods.original_img')->join('think_goods','think_goods.id = goods_stock.goods_id','left')->join('think_spec_goods','think_spec_goods.spec_id=goods_stock.spec_id','left')->where('goods_stock.stock','<',1000)->where('goods_stock.shop_id',$this->shopId)->select();
       //->where('goods_stock.shop_id',$this->shopId)
