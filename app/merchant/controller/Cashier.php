...
//        return json($map);
    ...
    public function getWorkerById(){
        ...
        if(!$info){
            return json(['code'=>0,'data'=>[],'msg'=>'No clerk found']);
        }
    ...
    public function getPayConfig($order_id,$pay_id=0,$auth_code=''){
        ...
        if(request()->isPost()){
            ...
            if($order_info->getData('order_status') != 1 || $order_info->getData('pay_status') != 0 ){
                return json(['code'=>0,'data'=>[],'msg'=>'current state cannot be paid']);
            }
            ...
            if(!key_exists($pay_id,$pay_arr)){
                return json(['code'=>0,'data'=>[],'msg'=>'current payment method does not exist']);
            }
            ...
            if($pay_arr[$pay_id]['type'] > 0 ){
                ...
                if($pay_arr[$pay_id]['id'] == 1){
                    ...
                    // Less than 0 means that payment is no longer required
                    if($order_info->order_amount <= 0){
                        ...
                        / / Determine whether the user balance is enough
                        ...
                        MoneyLog::operate($order_info->user_id,-$order_info->order_amount,21,1,'Deduct from store purchases',$this->userId);
                ...
                //WeChat payment
                    ...
                    $Payment = new Payment();
                    $res = $Payment->wx_bar($order_info['user_id'],'Store product payment','Store product payment',$order_id,$order_info['order_amount'],$auth_code,$this->shopId,' ');
                    if($res['code'] != 1){
                        return json(['code'=>0,'data'=>[],'msg'=>$res['msg']]);
                    }
                    $data = $res['data'];
    ...
    public function offlinePay($order_id,$pay_ids){
        if(request()->isPost()){
           ...
           // $pay_arr = $ShopPayConfigModel->where('shop_id','=',$shop_id)->whereOr("shop_id",'=',0)->column('id,name,type','id ');
            ...
            $pnme = count($pays) > 1?'Mixed payments':$pay_arr[key($pays)]['name'];
        ...
        if($this->request->isPost()){
            ...
            if($post_goods){
                ...
                foreach ($post_goods['spec_id'] as $k => $v){
                    ...
                    $num+= $data['goods_num'];
                    //TODO::The points of this place should be calculated according to the product settings. Now their points are the number of points for the price of the product.
            ...
            if($order_id){
                ...
                //It can only be modified if it has not been paid and submitted
                if($order_info->getData('order_status') != 0 || $order_info->getData('pay_status') != 0 ){
                    return json(['code'=>0,'data'=>[],'msg'=>'current state cannot be modified']);
                }
    ...
    public function complete(){
        ...
        $data=[
            ...
            "exchange_integral"=>$order_info['exchange_integral'],//How many points can be used to deduct
            "integral_money"=>sprintf("%.2f",$order_info['exchange_integral']/config('config.point_rate')),//How much can the points be deducted
        ...
        if($this->request->isPost()){
            ...
            if($integral && $order_info->exchange_integral && $order_info->order_amount){
                if($user['integral'] < $integral){
                   return json(['code'=>0,'data'=>[],'msg'=>'Insufficient number of points']);
                }
                //The deducted points are greater than the order can be used
            ...
            try {
               ...
               // subtract points
                if($order_info->user_id && $order_info->integral){
                    ...
                    IntegralLog::operate($order_info->user_id,-$order_info->integral,21,1,'Deduct from store purchases',$this->userId);
                }
                // subtract balance
                if($order_info->user_id && $order_info->user_money){
                    MoneyLog::setDec($order_info->user_id,$order_info->user_money);
                    MoneyLog::operate($order_info->user_id,-$order_info->user_money,21,1,'Deduct from store purchases',$this->userId);
                }
				// Check to erase zero
                if($moling){
                ...
                if($order_info->save()){
                    ...
                    Db::commit();
                  /* //Indicate that payment is no longer required
                    if($order_info->order_amount <= 0){
                        //The payment is directly changed to success
        ...
        try {
            if($order_info->getData('order_status') == 1){
                ...
                if($order_info->user_id && $order_info->integral){
                    ...
                    //$uid,$num,$act=1,$status=1,$remark='',$executor=0
                    IntegralLog::operate($order_info->user_id,$order_info->integral,22,1,'Refund if the store cancels the order',$this->userId);
                }
                //add balance
                if($order_info->user_id && $order_info->user_money){
                    MoneyLog::setInc($order_info->user_id,$order_info->user_money);
                    MoneyLog::operate($order_info->user_id,$order_info->user_money,22,1,'Refund if the store cancels the order',$this->userId);
                }
            }
        ...
        public function refunds(){
            ...
            try {
                ...
                if($order_info->user_id && $refunds_integral){
                    ...
                    //$uid,$num,$act=1,$status=1,$remark='',$executor=0
                    IntegralLog::operate($order_info->user_id,$refunds_integral,23,1,'Return of returned goods in store',$this->userId);
                ...
                if($order_info->user_id && $refunds_money){
                    ...
                    MoneyLog::operate($order_info->user_id,$refunds_money,23,1,'Refund of returned goods in store',$this->userId);
                ...
                if($refunds_price){
                    ...
                    $ShopBIllModel = new ShopBIllModel();
                    //Add to a payment reconciliation table
                    $ShopBIllModel->operate($this->userId,$this->userName,$shop_id,$order_id,4,'Cash receipts',-$refunds_price,$order_info['user_id'],$order_info['user_account' ],'Cash refund');
    ...
    public function dantui(){
        ...
        if($order_lists_info->returns_num >= $order_lists_info->goods_num){
            return json(['code'=>0,'data'=>[],'msg'=>'This item has been returned']);
        }
        ...
        //TODO:: The amount of money refunded Amount No judgment
        $order_info->returns_price=$order_info->returns_price+$refunds_price;
        ...
        try {
            ...
            //Record the current refund
            if($refunds_price){
                //Offline change the order status and set it to paid
                $ShopBIllModel = new ShopBIllModel();
                //Add to a payment reconciliation table
                $ShopBIllModel->operate($this->userId,$this->userName,$shop_id,$order_id,4,'Cash receipts',-$refunds_price,$order_info['user_id'],$order_info['user_account' ],'Offline refund');
            }
