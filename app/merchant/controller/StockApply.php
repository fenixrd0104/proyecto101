...
use app\merchant\model\StockApplyLogModel;
...
class StockApply extends Base
{
    ...
    public function fromAgree(){
        ...
        if(!$order_info || $order_info->getData('order_status') != 2 ){
            return json(['code'=>0,'data'=>[],'msg'=>'current state cannot agree']);
        }
   ...
   // reject application
    public function fromRefuse(){
        ...
        if(!$order_info || $order_info->getData('order_status') != 2 ){
            return json(['code'=>0,'data'=>[],'msg'=>'current state cannot be rejected']);
        }
