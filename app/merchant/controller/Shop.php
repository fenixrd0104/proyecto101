...
//        return json($this->shopId);
        $subshop = new ShopListsModel();

        $Nowpage = input('get.page') ? input('get.page'):1;
        $limits = input('get.limit',10);
        $count = $subshop->getAllCount($map);//Calculate the total pages
...
//        $id = input('param.id');
    ...
    public function shop_status()
    {
       $id = input('param.id');
        ...
        $status = Db::name('shop_lists')->where('id',$id)->where($map)->value('status');//Judge the current status
        if($status==1)
        {
            $flag = Db::name('shop_lists')->where('id',$id)->where($map)->update(['status'=>0]);
            return json(['code' => 1, 'data' => $flag['data'], 'msg' => 'banned']);
        }
        else
        {
            $flag = Db::name('shop_lists')->where('id',$id)->where($map)->update(['status'=>1]);
...
//            return json($param);
            $user_id=Db::name('member')->insertGetId($param);
...
//                $accdata[] = array(
//                    'uid'=> $user_id,
//                    'group_id'=> $v,//TODO::wrong group id
//                );
//            }
...
//            return json($param);
        ...
        $UserType = new UserType();
        $auth_lists = $UserType->getKeyVal(['shop_id'=>$shop_id]);
        $auth_checked =  Db::name('auth_group_access')->where(['uid'=>$id])->column('group_id');
        $info=Db::name('member')->where($map)->find();
//        return json($info);
        return json(['code'=>1,'data'=>['shop_lists'=>$shop_lists,'auth_lists'=>$auth_lists,'auth_checked'=>$auth_checked,'info'=>$info],'msg'=>'']);
    }
    public function status_staff(){
        ...
        $map['id']=$id;
        $status = Db::name('member')->where($map)->value('status');//Judging the current status
        if($status==1)
        {
            $flag = Db::name('member')->where($map)->update(['status'=>0]);
            return json(['code' => 1, 'data' => [], 'msg' => 'disabled']);
        }
        else
        {
            $flag = Db::name('member')->where($map)->update(['status'=>1]);
    ...
    public function payLists(){
        $paymsg=Db::name('shop_lists')->where('id',$this->shopId)->value('paymsg');
        if(!$paymsg){
            $paymsg=$this->paylist;
        }else{
            $paymsg=$paymsg;
            $paymsg=unserialize($paymsg);
        }
        return json(['code'=>1,'data'=>['lists'=>$paymsg],'msg'=>'']);
    ...
    public function payEdit(){
        $id = input('id');
        $shop_id = $this->shopId;
        $paymsg=Db::name('shop_lists')->where('id',$shop_id)->value('paymsg');
        $paymsg=unserialize($paymsg);
        $arr=[];
        foreach ($paymsg as $key =>$value){
            if($id=$value['id']){
                $arr=$value;
            }
        }
        if(request()->isPost()){
            $data=[];
            $data['name']=input('post.name');
            $edit_id=input('post.id');
            $data['status']=input('post.status',0);
            foreach ($paymsg as $key =>&$value){
                if($edit_id==$value['id']){
                    $value['status']=intval($data['status']);
                    $value['name']=$data['name'];
                }
            }

            if(Db::name('shop_lists')->where('id',$this->shopId)->update(['paymsg'=>serialize($paymsg)])){
                return json(['code'=>1,'data'=>$paymsg,'msg'=>'operation successful']);
            }else{
                return json(['code'=>0,'data'=>$paymsg,'msg'=>'operation failed']);
            }
        }
//        $info = $ShopPayConfigModel->where($map)->find();
    ...
    public function payStatus(){
        $id = input('id');
        $a=Db::name('shop_lists')->where('id',$this->shopId)->value('paymsg');
        if(!$a){
            $res=$this->paylist;
        ...
        Db::name('shop_lists')->where('id',$this->shopId)->update(['paymsg'=>serialize($res)]);
    ...
    * [role_state user state]
    ...
    //Product Sales Ranking Salesperson Ranking Return Statistics Inventory Statistics
    public function statistics(){
        ...
        //Top 10 products sold in 30 days
        ...
        //Top 10 Received Amounts in 30 Days
    ...
    public function detailed(){
        $shop_id = $this->shopId;
        $list = Db::name('shop_detailed')->where('shop_id',$shop_id)->order('id desc')->select();
        $count = Db::name('shop_detailed')->where('shop_id',$shop_id)->count();
        return json(['code' => 1, 'data' => ['lists'=>$list, 'count'=>$count], 'msg' => 'success']);
    }
