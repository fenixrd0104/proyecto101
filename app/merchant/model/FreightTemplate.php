...
class FreightTemplate extends Model
{
    ...
    public $charge_type=[
       ...
       function get_list($shopId){
       $list = Db::name("freight_template")->alias('ft')->join('shop_lists s','ft.shop_id=s.id','LEFT')->field('ft. *,s.name as shop_name')->where('shop_id',$shopId)->select()->toArray();
       foreach($list as $k=>$v){
